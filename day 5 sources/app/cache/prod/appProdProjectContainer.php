...
class appProdProjectContainer extends Container
{
    ...
    protected function getSecurity_Firewall_Map_Context_CustomerModuleService()
    {
        ...
        $g->add($a, array(0 => 'ROLE_USER'), NULL);
        $h = new \Symfony\Component\Security\Http\HttpUtils($e);
        $i = new \Symfony\Component\Security\Http\Firewall\LogoutListener($c, $h, '/customer/logout', '/customer/login', NULL);
        $i->addHandler(new \Symfony\Component\Security\Http\Logout\SessionLogoutHandler());
        return $this->services['security.firewall.map.context.customer_module'] = new \Symfony\Bundle\SecurityBundle\Security\FirewallContext(array(0 => new \Symfony\Component\Security\Http\Firewall\ChannelListener($g, new \Symfony\Component\Security\Http\EntryPoint\RetryAuthenticationEntryPoint(), $b), 1 => new \Symfony\Component\Security\Http\Firewall\ContextListener($c, array(0 => $this->get('security.user.provider.concrete.customers')), 'customer_module', $b, $d), 2 => $i, 3 => new \Symfony\Component\Security\Http\Firewall\UsernamePasswordFormAuthenticationListener($c, $f, new \Symfony\Component\Security\Http\Session\SessionAuthenticationStrategy('migrate'), $h, 'customer_module', array('check_path' => '/customer/auth', 'login_path' => '/customer/login', 'default_target_path' => '/customer/account', 'use_forward' => false, 'always_use_default_target_path' => false, 'target_path_parameter' => '_target_path', 'use_referer' => false, 'failure_path' => NULL, 'failure_forward' => false, 'username_parameter' => '_username', 'password_parameter' => '_password', 'csrf_parameter' => '_csrf_token', 'intention' => 'authenticate', 'post_only' => true), NULL, NULL, $b, $d), 4 => new \Symfony\Component\Security\Http\Firewall\AccessListener($c, $this->get('security.access.decision_manager'), $g, $f, $b)), new \Symfony\Component\Security\Http\Firewall\ExceptionListener($c, $this->get('security.authentication.trust_resolver'), $h, new \Symfony\Component\Security\Http\EntryPoint\FormAuthenticationEntryPoint($this->get('http_kernel'), $h, '/customer/login', false), NULL, NULL, $b));
    }
    ...
    {
        return $this->services['security.authentication.manager'] = new \Symfony\Component\Security\Core\Authentication\AuthenticationProviderManager(array(0 => new \Symfony\Component\Security\Core\Authentication\Provider\DaoAuthenticationProvider($this->get('security.user.provider.concrete.customers'), new \Symfony\Component\Security\Core\User\UserChecker(), 'customer_module', $this->get('security.encoder_factory'), true)));
    }
    ...
    {
        $this->services['security.user.provider.concrete.customers'] = $instance = new \Symfony\Component\Security\Core\User\InMemoryUserProvider();
        $instance->createUser(new \Symfony\Component\Security\Core\User\User('hhamon', 'secret', array(0 => 'ROLE_ADMIN')));
        $instance->createUser(new \Symfony\Component\Security\Core\User\User('jsmith', 'foobar', array(0 => 'ROLE_USER')));
        return $instance;
    }
